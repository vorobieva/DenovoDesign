...
class Blueprint:
    def __init__(self, blueprint_file = None, pdbfile = None, structure = None, segments = None, data=None):
        ...
        if blueprint_file or data:
            ...
            for remodel, bp_data in groupby(data, key = lambda x: x[3][0]):
                seg = None
                resdata = list(bp_data)
                self.bp_data += resdata
                if remodel == "R":
                    seg = None
                    for sstype, bp_data_ss in groupby(resdata, key = lambda y: y[2][0]):
                        id = sstype + str(segment_count[sstype])
                        resdata_ss = list(bp_data_ss)
                        segment_count[sstype] += 1
                        seg = Segment(id, sstype,  resdata_ss)
                        self.segments.append(seg)
                        self.segment_dict[id] = seg
                elif remodel == "." and self.structure: # assuming the non-remodeled part belong to the input structure.
                    id = "I1"
                    seg = Segment(id, "I", resdata)
                    self.segments.append(seg)
                    self.segment_dict[id] = seg
#                if sstype == "I" and self.structure:
#                    id = "I1" #sstype + str(segment_count[sstype])
##                    segment_count[sstype] += 1
#                    segment_residues = []
#                    for data in resdata:
...
#                    self.segments.append(seg)
#                    self.segment_dict[id] = seg
#                elif self.structure and sstype!="I":
#                    id = "I1"
#                    segment_residues = [ ]
#                    for data in resdata:
#                        segment_residues.append(residues[res_index])
